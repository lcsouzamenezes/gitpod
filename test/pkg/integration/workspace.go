...
func LaunchWorkspaceDirectly(it *Test, opts ...LaunchWorkspaceDirectlyOpt) (res *LaunchWorkspaceDirectlyResult) {
	...
	if workspaceImage == "" {
		it.t.Fatalf("cannot start workspaces without a workspace image (required by resolver)")
	}
	...
	if ideImage == "" {
		...
		if ideImage == "" {
			it.t.Fatalf("cannot start workspaces without an IDE image (required by resolver)")
		}
	}
