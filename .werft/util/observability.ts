...
export async function installMonitoringSatellite(params: InstallMonitoringSatelliteParams) {
    werft.log(sliceName, `cloning observability repository`)
    exec(`git clone --branch ${params.version} https://roboquat:$(cat /mnt/secrets/monitoring-satellite-preview-token/token)@github.com/gitpod-io/observability.git`, {silent: true})
    ...
    // dns_name needs to be set, but empty, to keep our code compilable
